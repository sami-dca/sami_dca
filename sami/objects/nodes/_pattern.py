...
def convert_hex_to_rgb(hx: str) -> tuple[int, int, int]:
    """
...
class Pattern(pydantic.BaseModel):
    seed: pydantic.conint(ge=0, le=9999, strict=True)
    colors: pydantic.conlist(
        item_type=pydantic.constr(
            to_lower=True,
            regex=r"[a-f0-9]{6}",  # noqa
            strict=True,
        ),
        min_items=3,
        max_items=9,
    ...
    shapes_count: pydantic.conint(ge=1, le=19, strict=True)
