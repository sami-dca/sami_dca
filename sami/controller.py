...
from .messages import Conversation, OwnMessage
...
def get_valid_conversations() -> List[Conversation]:
    ...
    existing_conversations = [
        Conversation.from_dbo(dbo) for dbo in existing_conversations_dbo
    ]
    return [
        conversation
...
def get_available_conversations() -> List[Conversation]:
    ...
    return [
        conversation
        for conversation in get_valid_conversations()
...
class Controller:
    ...
    class MainMenu(wxdisplay.MainMenu):
        ...
        def load_private_key(self, event: wx.FileDirPickerEvent) -> None:
            ...
            if not private_key_path.is_file():
                # File does not exist
                self.private_key_filePicker.SetPath("Specified file does not exist !")
                return

            rsa_private_key = PrivateKey.from_file(private_key_path)
            if rsa_private_key is None:
                # Invalid private key information in the file
                self.private_key_filePicker.SetPath("Invalid private key file !")
                return
        ...
        def export_pubkey(self, event: wx.CommandEvent) -> None:
            ...
            logger.info(f"Exported public key " f"to {str(self.keys_directory_path)!r}")
    ...
    class ReceivedMessages(wxdisplay.ReceivedMessages):
        ...
        def display_conversations(self, scroll_index: int, user_filter: str) -> None:
            ...
            conversations = [
                Conversation.from_dbo(conv_dbo) for conv_dbo in conversations_dbo
            ]
            ...
            for index, conv in enumerate(conversations):
                ...
                last_message_date = get_date_from_timestamp(last_message.time_received)

                node_sb_sizer = wx.StaticBoxSizer(
                    wx.StaticBox(self, id=wx.ID_ANY, label=conv_name), wx.HORIZONTAL
                )

                # Displays the last message of the conversation.
                conv_snippet_static_text = wx.StaticText(
                    node_sb_sizer.GetStaticBox(),
                    wx.ID_ANY,
                ...
                node_sb_sizer.Add(
                    conv_snippet_static_text, 1, wx.ALL | wx.ALIGN_CENTER_VERTICAL, 5
                )
                ...
                open_conversation_button_label = "Open"
                ...
                self.received_messages_bSizer.Add(
                    node_sb_sizer, 0, wx.ALL | wx.EXPAND, 5
                )

                # We're passing custom data using a lambda function
                conv_open_button.Bind(
                    wx.EVT_BUTTON, lambda event: self.open_chat_id(event, conv)
                )
            ...
            if displayed_conversations > 0 and False:  # Disabled
                ...
                self.chat_container_main_bSizer.Add(
                    scroll_bar, 0, wx.ALL | wx.EXPAND, 5
                )
        ...
        def show_new_message_box(self) -> None:
            ...
            new_chat_sb_sizer = wx.StaticBoxSizer(
                wx.StaticBox(self, wx.ID_ANY, "New conversation"), wx.VERTICAL
            )

            self.recipient_bSizer = wx.BoxSizer(wx.HORIZONTAL)

            recipient_static_text = wx.StaticText(
                new_chat_sb_sizer.GetStaticBox(),
                wx.ID_ANY,
                "Send to:",
                wx.DefaultPosition,
                wx.DefaultSize,
            ...
            self.recipient_bSizer.Add(
                recipient_static_text, 0, wx.ALIGN_CENTER_VERTICAL, 5
            )
        ...
        def open_chat_id(
            self, event: wx.CommandEvent, conversation: Conversation
        ) -> None:
        ...
        def display_conversation(self) -> None:
            for message in self.conversation.messages:
                ...
                message_sb_sizer = wx.StaticBoxSizer(
                    wx.StaticBox(self, wx.ID_ANY, message.author.name), wx.HORIZONTAL
                )

                message_content_static_text = wx.StaticText(
                    message_sb_sizer.GetStaticBox(),
                    wx.ID_ANY,
                    message.content,
                    wx.DefaultPosition,
                    wx.DefaultSize,
                ...
                message_sb_sizer.Add(
                    message_content_static_text, 1, wx.ALIGN_CENTER_VERTICAL, 5
                )
