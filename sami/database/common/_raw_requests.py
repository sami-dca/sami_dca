...
class RawRequestsDatabase(CommonDatabaseTemplate):
    def is_known(self, uid: Identifier) -> bool:
        with self._init_session() as session:
            return session.query(exists().where(RequestDBO.uid == uid)).scalar()
    ...
    def get_all_since(self, timestamp: int) -> List[RequestDBO]:
        with self._init_session() as session:
            requests = (
                session.query(RequestDBO).where(RequestDBO.timestamp > timestamp).all()
            )
            session.expunge_all()
        return requests
    ...
    def get_last_received(self) -> Optional[RequestDBO]:
        with self._init_session() as session:
            q = session.query(RequestDBO).where(max(RequestDBO.timestamp)).one_or_none()
            session.expunge_all()
        return q
    ...
    def purge_old(self, lifespan: int = settings.max_request_lifespan) -> None:
        ...
        with self._init_session() as session:
            requests = session.query(RequestDBO).where(RequestDBO.timestamp < threshold)
            for dbo in requests:
                session.delete(dbo)
